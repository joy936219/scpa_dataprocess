...
$sql1='';
if($NP=='0'){
    $sql1=sprintf('SELECT * FROM estimates JOIN onlyone ON estimates.SchoolName=onlyone.SchoolName WHERE estimates.ThisYearAdmission>=%d-20 AND estimates.ThisYearAdmission<%d+20 AND estimates.Type=\'%s\' AND estimates.Area LIKE \'%%%s%%\' ORDER BY estimates.Sort ASC',mysql_real_escape_string($Total1),mysql_real_escape_string($Total1),mysql_real_escape_string($Type),mysql_real_escape_string($Area));
}else{
    if($NP=='1'){
        $sql1=sprintf('SELECT * FROM estimates JOIN onlyone ON estimates.SchoolName=onlyone.SchoolName WHERE estimates.ThisYearAdmission>=%d-20 AND estimates.ThisYearAdmission<%d+20 AND estimates.Type=\'%s\' AND estimates.Area LIKE \'%%%s%%\' AND onlyone.NP IN(\'1\',\'4\',\'5\') ORDER BY estimates.Sort ASC',mysql_real_escape_string($Total1),mysql_real_escape_string($Total1),mysql_real_escape_string($Type),mysql_real_escape_string($Area));
    }else{
        if($NP=='2'){
            $sql1=sprintf('SELECT * FROM estimates JOIN onlyone ON estimates.SchoolName=onlyone.SchoolName WHERE estimates.ThisYearAdmission>=%d-20 AND estimates.ThisYearAdmission<%d+20 AND estimates.Type=\'%s\' AND estimates.Area LIKE \'%%%s%%\' AND onlyone.NP IN(\'2\',\'3\',\'6\') ORDER BY estimates.Sort ASC',mysql_real_escape_string($Total1),mysql_real_escape_string($Total1),mysql_real_escape_string($Type),mysql_real_escape_string($Area));
        }
    }
}
